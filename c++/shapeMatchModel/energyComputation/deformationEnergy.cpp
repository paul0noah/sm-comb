...
void DeformationEnergy::useCustomDeformationEnergy(const Eigen::MatrixXf& Vx2VyCostMatrix, bool useAreaWeighting, bool membraneReg, float lambda) {
    ...
    if (useTranspose) {
        for (int j = 0; j < FaCombo.rows(); j++) {
            for (int i = 0; i < 3; i++) {
                temp(j, i) = Vx2VyCostMatrix(FbCombo(j, i), FaCombo(j, i));
            }
        }
    }
    else {
        for (int j = 0; j < FaCombo.rows(); j++) {
            for (int i = 0; i < 3; i++) {
                temp(j, i) = Vx2VyCostMatrix(FaCombo(j, i), FbCombo(j, i));
            }
        }
    }
...
void DeformationEnergy::prune(const Eigen::VectorX<bool>& pruneVec) {
    ...
    for (long i = 0; i < defEnergy.rows(); i++) {
        if (pruneVec(i)) {
            defEnergy(elementCounter, 0) = defEnergy(i, 0); // this works bc always elementCounter <= i
            elementCounter++;
            if (true) {
    ...
    defEnergy.conservativeResize(elementCounter, 1);
}
